...
class ProgramaPrincipal:
    ...
    def crearTablas(self):
        ...
        conexion.miCursor.execute("CREATE TABLE HISTORICO_MONO (id_mono INTEGER PRIMARY KEY , marca VARCHAR(30), precio FLOAT NOT NULL, cantidadDisponibles INTEGER NOT NULL, potencia VARCHAR(30), color VARCHAR(30), fecha DATETIME)")
        conexion.miConexion.commit()
...
########### FUNCION PARA CARGAR ELEMENTOS A LA LISTA
    def cargar_monopatin(self):
        ...
        try:
            ...
            conexion.miConexion.commit()
            conexion.miCursor.execute("INSERT INTO HISTORICO_MONO(marca,precio,cantidadDisponibles,potencia,color,fecha) VALUES('{}','{}','{}', '{}', '{}', '{}')".format(self.marca,self.precio, self.cantidadDisponibles,self.potencia, self.color, self.fecha))
            conexion.miConexion.commit()
...
######## FUNCION PARA BORRAR UN ELEMENTO DE LA LISTA
    def borrar_monopatin(self):
        ...
        try:
            ...
            conexion.miCursor.execute("SELECT * FROM HISTORICO_MONO where id_mono='{}'".format(identificador))
            if conexion.miCursor.fetchone():
                conexion.miCursor.execute("delete from HISTORICO_MONO where id_mono='{}' ".format(identificador))
                conexion.miConexion.commit()
